...
public class _URL {
	...
	public _URL(String urlAsString) {
	...
	public void changeParameter(String parName, String value) {
		...
		if (ind > -1) {
			...
			if (endInd != ind) {
				urlAsString = urlAsString.substring(0, ind) + parName + "=" + value
						+ urlAsString.substring(endInd + 1, urlAsString.length());
			} else {
				urlAsString = urlAsString.substring(0, ind) + parName + "=" + value
						+ urlAsString.substring(endInd + parName.length() + value.length() - 1, urlAsString.length());
			}
		} else {
