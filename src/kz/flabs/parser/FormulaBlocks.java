...
public final class FormulaBlocks implements Const{
	...
	private static final Pattern sysFieldsPattern = Pattern.compile("^\\(?(form|author|docid|viewtext|viewnumber|regdate|lastupdate|ddbid|parentdocddbid|parentdocid|parentdoctype|has_response)");
	...
	public void parse(String formula){	
		...
		while (matcher.find()){
			...
			blocks.add(block);
			if(block.paramatrized) paramatrizedQuery = true;
		}
	}
	...
	public void putParameters(Map<String, String[]> fields){
		for(Block block:blocks){
			for(FieldExpression fieldExpr:block.getParamatrizedExpressList().values()){
				...
				if (parValue != null){		
					try {
						fieldExpr.fieldValue = new String(((String)parValue[0]).getBytes("ISO-8859-1"),"UTF-8").replace("'", "''");						
	...
	public static void main(String[] args){
		try {
			...
			//String formula = "form = 'accrual' or (typeoper = 'operations' or calcstaff#number = 1)";
			//String formula = "form = 'accrual' or (form = 'operations' and calcstaff#number = 1)";
			//String formula = "form = 'demand'";
			//String formula = "form = 'demand' and allcontrol <> 'reset' and status <> 'notActual'";
				//String formula = "form = 'accrual' or (form = 'operations' and calcstaff = '1' )";
			String formula = "vnnumber ~ '$vnnumber' and form ~ '$form' and (docversion > 1 or isrejected = 1)";
			System.out.println(formula + "\n");			
			//FormulaBlocks blocks = new FormulaBlocks(formula,DocType.DOCUMENT);	
			...
			IQueryFormula queryFormula = env.getDataBase().getQueryFormula("", blocks);


			System.out.println(queryFormula + "\n");
			System.out.println("SQL = " + queryFormula.getSQL(new HashSet<String>(Arrays.asList(Const.observerGroup))));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
