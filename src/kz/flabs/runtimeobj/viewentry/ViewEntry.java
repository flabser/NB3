...
public class ViewEntry implements IViewEntry, Const{
	...
	public ViewEntry(IDatabase db, ResultSet rs, Set<DocID> toExpandResponses, User user) throws SQLException{
        ...
        if (toExpandResponses.contains(new DocID(docID, docType))) {
            String userID = user != null ? user.getUserID() : Const.sysUser;
            Set<String> userGroups = user != null ? user.getAllUserGroups() : Const.sysGroupAsSet;
            descendants = db.getDescendants(docID, docType, null, 0, userGroups, userID).xmlContent;
        }
		try {
	...
	@Deprecated
	public ViewEntry(IDatabase db, ResultSet rs, Set<DocID> toExpandResponses, User user, SimpleDateFormat simpleDateFormat, String responseQueryCondition) throws SQLException{
		this.db = db;
		type = ViewEntryType.DOCUMENT;
		docID = rs.getInt("DOCID");
		docType = rs.getInt("DOCTYPE");
        ...
        if (total_resp > 0) {
            try {
                int count_resp = db.getDocumentsCountByCondition(responseQueryCondition + (!"".equalsIgnoreCase(responseQueryCondition) ? " and " : "") + " parentdocid = " + docID, Const.supervisorGroupAsSet, Const.sysUser);
                if (count_resp > 0) {
                    hasResp = 1;
                }
            } catch (Exception e) {
        ...
        if (toExpandResponses.contains(new DocID(docID, docType))) {
            String userID = user != null ? user.getUserID() : Const.sysUser;
            Set<String> userGroups = user != null ? user.getAllUserGroups() : Const.supervisorGroupAsSet;
            descendants = db.getDescendants(docID, docType, null, 0, userGroups, userID, responseQueryCondition).xmlContent;
        }
	...
	public boolean equals(Object o) {
		...
		if (!(o instanceof ViewEntry)) return false;

		ViewEntry viewEntry = (ViewEntry) o;

		if (!ddbID.equals(viewEntry.ddbID)) return false;
