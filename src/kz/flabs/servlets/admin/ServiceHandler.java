...
public class ServiceHandler implements Const {
	...
	String getSettings(AppEnv env) {
		...
		xmlFragment.append("<entrypoint>" + env.globalSetting.id + File.pathSeparator + env.globalSetting.entryPoint.replace("&", "&amp;")
		        + "</entrypoint>");
		xmlFragment.append("<defaultredirecturl>" + env.globalSetting.id + File.pathSeparator
		        + env.globalSetting.defaultRedirectURL.replace("&", "&amp;") + "</defaultredirecturl>");
		...
		if (env.globalSetting.databaseEnable) {
			...
			xmlFragment.append("<daemons>");
			xmlFragment
			        .append("<cyclecontrol><mininterval>" + env.globalSetting.cycleContrSchedSetings.minInterval + "</mininterval></cyclecontrol>");
			xmlFragment.append("</daemons>");
	...
	String getMainDoc(String dbid, String docid) throws DocumentException, DocumentAccessException, ComplexObjectException {
		...
		xmlFragment += "<docid>" + doc.getDdbID() + "</docid>" + "<author>" + doc.getAuthorID() + "</author>" + "<regdate>"
		        + Util.convertDataTimeToString(doc.getRegDate()) + "</regdate>" + "<lastupdate>" + Util.convertDataTimeToString(doc.getLastUpdate())
		        + "</lastupdate>" + "<form>" + doc.form + "</form>" + "<parentdocid>" + doc.getParentDocumentID() + "</parentdocid>";
		...
		/*
		 * xmlFragment = "<document doctype=\"" + doc.docType + "\" docid=\""+
		 * doc.getDdbID() + "\" form= \""+ doc.form + "\" dbid= \""+ dbid +
		 * "\" " + "parentdocid = \"" + doc.getParentDocumentID() + "\" " +
		 * "author=\"" + doc.getAuthorID() +"\" " + "regdate=\"" +
		 * Util.convertDataTimeToString(doc.getRegDate()) +"\" " +
		 * "lastupdate=\"" + Util.convertDataTimeToString(doc.getLastUpdate())
	...
	String getLogsListWrapper(ArrayList<File> fl, int pageNum) {
		...
		int length = fl.size();
		/*
		 * int pageNumMinusOne = pageNum; pageNumMinusOne--; int startEntry =
		 * pageNumMinusOne * pageSize; int endEntry = startEntry + pageSize; int
		 * num = 1;
		 */
		...
		return "<query count=\"" + fl.size() + "\" currentpage=\"" + pageNum + "\" maxpage=\"" + maxPage + "\" path=\"" + logs.logDir + "\">"
		        + fieldsAsXML + "</query>";
	}
	...
	String getBackupListWrapper(ArrayList<File> fl) {
		...
		while (it.hasNext()) {
			...
			fieldsAsXML += "<entry><name>"
			        + fileName
			        + "</name>"
			        + "<length>"
			        + Util.convertBytesToKilobytes(logFile.length())
			        + "</length>"
			        + (fileName.split("_").length > 2 ? "<app>" + fileName.substring(fileName.indexOf("_") + 1, fileName.lastIndexOf("_")) + "</app>"
			                : "") + "<lastmodified>" + Util.dateTimeFormat.format(logFile.lastModified()) + "</lastmodified>" + "</entry>";
		}
	...
	public String getCalendarListWrapper(HolidayCollection holidays, int i, int j) {
		...
		for (Holiday h : holidays.holidays) {
			fieldsAsXML += "<entry><title>" + h.getTitle() + "</title></entry>" + "<startdate>" + Util.convertDataToString(h.getStartDate())
			        + "</startdate>" + "<enddate>" + Util.convertDataToString(h.getEndDate()) + "</enddate>" + "<country>" + h.getCountry()
			        + "</country></entry>";
		}
