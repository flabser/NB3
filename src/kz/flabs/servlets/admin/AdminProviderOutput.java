...
public class AdminProviderOutput extends ProviderOutput {
	...
	private static final String adminXSLTPath = "." + File.separator + "webapps" + File.separator + "Administrator" + File.separator + "xslt" + File.separator;	
	...
	public String getStandartOutput(){			
		...
		if (userSession != null && userSession.currentUser.getAppUser() != null) {
			localUserName = userSession.currentUser.getAppUser().getFullName();			
		} else {
			localUserName = userSession.currentUser.getUserID();
		}

		return xmlTextUTF8Header + "<request type=\"" + type + "\" lang=\"RUS\" element=\"" + element + "\" id=\"" + id + "\" " +
		"useragent=\"" + browser + "\"  skin=\"pchelka\" dbid=\"" + dbID + "\" userid=\"" + userSession.currentUser.getUserID() + "\" " +
