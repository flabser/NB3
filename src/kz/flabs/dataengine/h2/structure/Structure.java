...
public class Structure extends DatabaseCore implements IStructure, Const {
    ...
    private static final String empSelectFields = "e.EMPID, e.DEPID, e.BOSSID, e.ORGID, e.USERID," +
            ...
            " e.PARENTDOCID, e.PARENTDOCTYPE, e.ISBOSS, e.INDEXNUMBER, e.RANK, e.PHONE, e.SENDTO, e.OBL, e.REGION, e.VILLAGE, e.BIRTHDATE, e.STATUS, e.VIEWNUMBER, e.VIEWDATE, " + DatabaseUtil.getViewTextList("e");
    ...
    public StringBuffer getStructure(Set<DocID> toExpand) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                xmlContent.append("<entry  doctype=\"" + DOCTYPE_ORGANIZATION + "\" " +
                        "docid=\"" + orgID + "\" " + XMLUtil.getAsAttribute("viewtext", viewText) +
                        "url=\"Provider?type=organization&amp;id=o&amp;key=" + orgID + "\"" + respAttr + "><viewtext>" + XMLUtil.getAsTagValue(viewText) + "</viewtext><responses>");
    ...
    public StringBuffer getExpandedStructure() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String viewText = rs.getString("VIEWTEXT");
                xmlContent.append("<entry  doctype=\"" + DOCTYPE_ORGANIZATION + "\" " +
                        "dbid=\"" + db.getDbID() + "\" docid=\"" + orgID + "\" " + XMLUtil.getAsAttribute("viewtext", viewText) +
                        "url=\"Provider?type=structure&amp;id=organization&amp;key=" + orgID + "&amp;dbid=" + db.getDbID() + "\"><viewtext>" + XMLUtil.getAsTagValue(viewText) + "</viewtext><responses>");
    ...
    private StringBuffer getAllEntries(int docID, int docType, Connection conn) {
        ...
        try {
            ...
            String sql = "SELECT DEPID id, NULL userid, DOCTYPE, VIEWTEXT, RANK, VIEWTEXT1, VIEWTEXT2 " +
                    " FROM DEPARTMENTS d" +
                    " WHERE d.PARENTDOCID = " + docID + " AND d.PARENTDOCTYPE = " + docType +
                    " UNION" +
                    " SELECT EMPID id, USERID, DOCTYPE, VIEWTEXT, RANK, VIEWTEXT1, VIEWTEXT2 " +
                    " FROM EMPLOYERS e" +
                    " WHERE e.PARENTDOCID = " + docID + " AND e.PARENTDOCTYPE = " + docType +
            ...
            while (rs.next()) {
                ...
                if (type == DOCTYPE_EMPLOYER)
                    ...
                    typeStr = "department";
                xmlContent.append("<entry doctype=\"" + type + "\" " +
                        "dbid=\"" + db.getDbID() + "\" docid=\"" + id + "\" " + XMLUtil.getAsAttribute("viewtext", viewText) +
                        "url=\"Provider?type=structure&amp;id=" + typeStr + "&amp;key=" + id + "\"><viewtext>" + XMLUtil.getAsTagValue(viewText)
                        + "</viewtext><viewtext1>" + XMLUtil.getAsTagValue(viewText1) + "</viewtext1><viewtext2>" + XMLUtil.getAsTagValue(viewText2) + "</viewtext2>");
                if (type == DOCTYPE_EMPLOYER)
    ...
    public Employer getAppUser(String user) {
        ...
        try {
            conn.setAutoCommit(false);
            Statement s = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
            String sql = "SELECT " + empSelectFields + " FROM EMPLOYERS e WHERE e.USERID = '" + user + "'";
            ResultSet rs = s.executeQuery(sql);
            if (rs.next()) {
                ...
                User systemUser = sysDb.getUser(user);
                if (systemUser.isSupervisor()) {
    ...
    public int insertOrganization(Organization doc) {
        ...
        try {
            conn.setAutoCommit(false);
            String orgSQL = "INSERT INTO ORGANIZATIONS (" +
                    " AUTHOR," +
                    " REGDATE, " +
                    " DOCTYPE," +
                    " PARENTDOCID," +
            ...
            pst = conn.prepareStatement("INSERT INTO STRUCTURE_TREE_PATH (ANCESTOR, DESCENDANT, LENGTH) " +
                    " SELECT stp.ANCESTOR, '" + ddbid + "', stp.LENGTH + 1 FROM STRUCTURE_TREE_PATH as stp WHERE stp.DESCENDANT = '" + doc.getParentDocumentID() + "' UNION ALL SELECT '" + ddbid + "', '" + ddbid + "', 0");
            pst.executeUpdate();
            conn.commit();
            pst.close();
            return key;
    ...
    public int updateOrganization(Organization doc) {
        ...
        try {
            ...
            String updateEmp = "update ORGANIZATIONS set " +
                    "LASTUPDATE = '" + Database.sqlDateTimeFormat.format(new Date()) + "', " +
                    "VIEWTEXT = '" + doc.getViewText().replace("'", "''") + "', VIEWICON = '" + doc.getViewIcon() + "', " +
                    "FULLNAME = '" + doc.getFullName().replace("'", "''") + "', " +
                    "DDBID = '" + doc.getDdbID() + "', " +
                    "ISMAIN = " + doc.getIsMain() + ", " +
                    "BIN = '" + doc.getBIN() + "', " +
                    "SHORTNAME = '" + doc.getShortName().replace("'", "''") + "', COMMENT = '" + doc.getComment().replace("'", "''") + "', " +
                    "ADDRESS = '" + doc.getAddress().replace("'", "''") + "', " +
                    "VIEWNUMBER = " + doc.getViewNumber() + ", " +
                    "VIEWDATE = " + (doc.getViewDate() != null ? "'" + Database.sqlDateTimeFormat.format(doc.getViewDate()) + "'" : "null") + ", " +
                    viewTextList +
                    " where ORGID = " + doc.getDocID();
            statProject.executeUpdate(updateEmp);
            statProject.close();
            updateEmp = "DELETE FROM STRUCTURE_TREE_PATH " +
                    " WHERE DESCENDANT IN (SELECT DESCENDANT FROM STRUCTURE_TREE_PATH WHERE ANCESTOR = '" + doc.getDdbID() + "') " +
                    " AND ANCESTOR IN (SELECT ANCESTOR FROM STRUCTURE_TREE_PATH WHERE DESCENDANT = '" + doc.getDdbID() + "' AND ANCESTOR != DESCENDANT)";
            ...
            updateEmp = "INSERT INTO STRUCTURE_TREE_PATH (ANCESTOR, DESCENDANT, LENGTH) " +
                    ...
                    " FROM STRUCTURE_TREE_PATH as supertree " +
                    " CROSS JOIN STRUCTURE_TREE_PATH as subtree " +
                    " WHERE supertree.descendant = '" + doc.getParentDocumentID() +
                    "' AND subtree.ancestor = '" + doc.getDdbID() + "'";
    ...
    public int insertDepartment(Department doc) {
        ...
        try {
            conn.setAutoCommit(false);
            String depSQL = "INSERT INTO DEPARTMENTS (" +
                    " AUTHOR," +
                    " REGDATE, " +
                    " DOCTYPE," +
                    " PARENTDOCID," +
                    ...
                    " VIEWNUMBER, " +
                    " VIEWDATE, " +
                    DatabaseUtil.getViewTextList("") + ") VALUES ('" +
                    doc.getAuthorID()
                    + "', '" +
                    Database.sqlDateTimeFormat.format(new Date())
                    + "', " +
    ...
    public int updateDepartment(Department doc) {
        ...
        try {
            ...
            String updateEmp = "update DEPARTMENTS set " +
                    "LASTUPDATE = '" + Database.sqlDateTimeFormat.format(doc.getLastUpdate()) + "', " +
                    "VIEWTEXT = '" + doc.getViewText().replace("'", "''") + "', VIEWICON = '" + doc.getViewIcon() + "', ORGID = " + (doc.getOrgID() == 0 ? "null" : doc.getOrgID()) + ",  " +
                    "MAINID = " + (doc.getMainID() == 0 ? "null" : doc.getMainID()) + ", EMPID = " + (doc.getEmpID() == 0 ? "null" : doc.getEmpID()) + ", " +
                    "FULLNAME = '" + doc.getFullName().replace("'", "''") + "', " +
                    "SHORTNAME = '" + doc.getShortName().replace("'", "''") + "', COMMENT = '" + doc.getComment().replace("'", "''") + "', " +
    ...
    public Department getDepartment(int id, User user) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (group.isValid) groups.add(group);
            }
    ...
    public StringBuffer getResponses(int docID, int docType) {
        ...
        try {
            ...
            String sql = "select depid, doctype, viewtext, form, 'department' as name from departments where parentdocid = " + docID +
                    ...
                    " select empid, doctype, viewtext, form, 'employer' as name from employers where parentdocid = " + docID + " and parentdoctype = " + docType;
            ...
            while (rs.next()) {
                ...
                xmlContent.append("<entry doctype=\"" + type + "\" " +
                        "docid=\"" + id + "\" " + XMLUtil.getAsAttribute("viewtext", viewText) +
    ...
    public StringBuffer getDepartments(int docID, int docType, Set<DocID> toExpand) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                xmlContent.append("<entry  doctype=\"" + DOCTYPE_DEPARTMENT + "\" " +
                        "docid=\"" + depID + "\" " + XMLUtil.getAsAttribute("viewtext", viewText) +
    ...
    public int insertEmployer(Employer doc) throws LicenseException {
        ...
        try {
            ...
            String empSQL = "insert into EMPLOYERS (" +
                    " AUTHOR," +
                    " REGDATE, " +
                    " DOCTYPE," +
                    " PARENTDOCID," +
    ...
    public int updateEmployer(Employer doc) {
        ...
        try {
            ...
            String updateEmp = "UPDATE EMPLOYERS SET " +
                    "LASTUPDATE = '" + Database.sqlDateTimeFormat.format(new Date()) + "', " +
                    ...
                    "PARENTDOCTYPE = " + doc.parentDocType + ", PARENTDOCID = " + doc.parentDocID + ", " +
                    "OBL = " + doc.getObl() + ", REGION = " + doc.getRegion() + ", VILLAGE = " + doc.getVillage() + ", " +
                    "ORGID = " + (doc.getOrgID() == 0 ? "null" : doc.getOrgID()) + ", DEPID = " + (doc.getDepID() == 0 ? "null" : doc.getDepID()) + ", " +
                    "BOSSID = " + (doc.getBossID() == 0 ? "null" : doc.getBossID()) + ", POST = " + (doc.getPostID() == 0 || doc.getPostID() == 999 ? "null" : doc.getPostID()) + ", " +
                    "SHORTNAME = '" + doc.getShortName() + "', COMMENT = '" + doc.getComment() + "', USERID = '" + doc.getUserID() + "', " +
                    "BIRTHDATE = " + (doc.getBirthDate() != null ? "'" + Database.sqlDateTimeFormat.format(doc.getBirthDate()) + "'" : "null") + ", " +
                    "HITS = " + doc.getHits() + ", STATUS = " + doc.getStatus() + ", " +
                    "VIEWNUMBER = " + doc.getViewNumber() + ", " +
                    "VIEWDATE = " + (doc.getViewDate() != null ? "'" + Database.sqlDateTimeFormat.format(doc.getViewDate()) + "'" : "null") + ", " +
                    viewTextList +
            ...
            for (String appID : doc.getUser().enabledApps.keySet()) {
                updateEmp = "DELETE FROM USER_ROLES WHERE EMPID = " + doc.getDocID() + " AND TYPE = " + Const.DOCTYPE_EMPLOYER + " AND APPID = '" + appID + "'";
                statement.executeUpdate(updateEmp);
                //      updateEmp = "DELETE FROM USER_ROLES WHERE EMPID = " + doc.getDocID() + " AND TYPE = " + Const.DOCTYPE_EMPLOYER;
                //     statement.executeUpdate(updateEmp);
            }
    ...
    public void fillBlobs(Connection conn, BaseDocument doc, String customBlobTableSuffix) throws SQLException {
        ...
        Statement statement = conn.createStatement();
        ResultSet rs = statement.executeQuery("select * from CUSTOM_BLOBS_" + customBlobTableSuffix + "  where CUSTOM_BLOBS_" + customBlobTableSuffix + ".DOCID = " + doc.getDocID() + " ORDER BY ID");
        while (rs.next()) {
    ...
    public int insertGroup(UserGroup doc) {
        ...
        try {
            ...
            String groupSQL = "INSERT INTO GROUPS (" +
                    "FORM, " +
                    "GROUPNAME, " +
                    "OWNER, " +
                    "DESCRIPTION, " +
    ...
    public int updateGroup(UserGroup doc) {
        ...
        try {
            conn.setAutoCommit(false);
            Date viewDate = doc.getViewDate();
            /*String updateGroup = "UPDATE GROUPS SET " +
					"GROUPNAME = '" + doc.getName() + "', DESCRIPTION = '" + doc.getDescription().replace("'", "''") + "', OWNER = '" +
					doc.getOwner() + "', DEFAULTRULEID = '" + doc.getDefaultRuleID() + "', VIEWTEXT1 = '" +
					doc.getViewTextList().get(0).replace("'", "''") + "', VIEWTEXT2 = '" +doc.getViewTextList().get(1).replace("'", "''") + 
					"', VIEWTEXT3 = '" + doc.getViewTextList().get(2).replace("'", "''") + "', VIEWNUMBER = " + doc.getViewNumber() + 
					", VIEWDATE = " + (viewDate != null ? "'" + new Timestamp(viewDate.getTime()) + "'" : "null") + " WHERE GROUPID = " + doc.getDocID();*/
    ...
    public Employer fillEmpDoc(ResultSet rs, Connection conn) throws SQLException {
        ...
        while (groupResultSet.next()) {
            ...
            if (group.isValid) emp.addGroup(group);
        }
        ...
        while (filterResultSet.next()) {
            ...
            if (filter.isValid) emp.addFilter(filter);
        }
    ...
    protected String getEmployerEntry(ResultSet rs) throws SQLException {
        ...
        String value = "<entry  doctype=\"" + DOCTYPE_EMPLOYER + "\" " +
                "docid=\"" + empID + "\" " +
    ...
    private UserGroup fillGroupDoc(ResultSet rs) {
        ...
        try {
            ...
            conn.setAutoCommit(false);
            Statement st = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
            ResultSet res = st.executeQuery("SELECT EMPID FROM USER_GROUPS WHERE GROUPID = " + group.getDocID() + " AND TYPE = " + Const.DOCTYPE_EMPLOYER);
            while (res.next()) {
            ...
            ResultSet roleResultSet = rolest.executeQuery("SELECT * FROM USER_ROLES ur WHERE ur.EMPID = " + group.getDocID() + " AND ur.TYPE = " + group.docType);
            while (roleResultSet.next()) {
    ...
    private boolean hasResponse(int docID, int docType) {
        ...
        try {
            conn.setAutoCommit(false);
			/*if (docType == DOCTYPE_ORGANIZATION){
				...
				statement.close();
			}else if(docType == DOCTYPE_DEPARTMENT){
				String sql = "select count(EMPID) FROM EMPLOYERS e where e.PARENTDOCID=" + docID + " AND e.PARENTDOCTYPE=" + docType; 
				Statement statement = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
				ResultSet rs = statement.executeQuery(sql);
				if (rs.next()) {
				...
				statement.close();
			}else if(docType == DOCTYPE_EMPLOYER) {
				String sql = "select count(DEPID) FROM DEPARTMENTS d where d.PARENTDOCID=" + docID + " AND d.PARENTDOCTYPE=" + docType;
				Statement statement = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
				ResultSet rs = statement.executeQuery(sql);
				if (rs.next()) {
					int count = rs.getInt(1);
