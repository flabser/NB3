package kz.flabs.dataengine.mssql.filters;
...
/*	@Override
	public int addToFavorites(int docID, int docType, String userName) {
		Connection conn = dbPool.getConnection();
		int key;
		try {
			String sql = "UPDATE " + DatabaseUtil.getReadersTableName(docType) + " SET FAVORITES = 1 WHERE DOCID = ? AND USERNAME = ?";
	...
	@Override
	public int removeFromFavorites(int docID, int docType, String userName) {
		Connection conn = dbPool.getConnection();
		int key;
		try {
			PreparedStatement pst = conn.prepareStatement("UPDATE " + DatabaseUtil.getReadersTableName(docType) + " SET FAVORITES = 0 WHERE DOCID = ? AND USERNAME = ?");
			pst.setInt(1, docID);
			pst.setString(2, userName);
			key = pst.executeUpdate();
			conn.commit();
...
/*	@Override
	public StringBuffer getFavorites(IQueryFormula nf, Set<String> complexUserID, String absoluteUserID, int offset, int pageSize, String fieldsCond, Set<DocID> toExpandResponses, Set<String> toExpandCategory, TagPublicationFormatType publishAs) {
		StringBuffer xmlContent = new StringBuffer(10000);
		Connection conn = dbPool.getConnection();
		try {
			Statement s = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);			
			String sql = "select has_attachment, form, docid, doctype, viewtext, viewtext1, viewtext2, viewtext3, viewnumber, viewdate from maindocs where docid in (select docid from readers_maindocs where username = '" + absoluteUserID + "' and favorites = 1) " +
					" union " +
					" select has_attachment, form, docid, doctype, viewtext, viewtext1, viewtext2, viewtext3, viewnumber, viewdate  from tasks where docid in (select docid from readers_tasks where username = '" + absoluteUserID + "' and favorites = 1) " +
					" union " +
					" select has_attachment, form, docid, doctype, viewtext, viewtext1, viewtext2, viewtext3, viewnumber, viewdate  from executions where docid in (select docid from readers_executions where username = '" + absoluteUserID + "' and favorites = 1) " +
					" union " +
					" select has_attachment, form, docid, doctype, viewtext, viewtext1, viewtext2, viewtext3, viewnumber, viewdate  from projects where docid in (select docid from readers_projects where username = '" + absoluteUserID + "' and favorites = 1) " +
					" order by " + nf.getSortBlock().fieldName + "  " + nf.getSortBlock().order + " LIMIT " + pageSize + " OFFSET " + offset;
			ResultSet rs = s.executeQuery(sql);			
			while (rs.next()) {
				xmlContent.append(db.getDocumentEntry(conn, complexUserID, absoluteUserID, rs, fieldsCond, toExpandResponses));
			}		
			s.close();
			rs.close();
			conn.commit();
...
/*	@Override 
	public int isFavourites(Connection conn, int docID, int docType, String userName) {
		int isFavour = 0;
		try {
			String sql = "SELECT favorites from " + DatabaseUtil.getReadersTableName(docType) + " where docid = ? and username = ?";
			PreparedStatement pst = conn.prepareStatement(sql);
			pst.setInt(1, docID);
			pst.setString(2, userName);
			ResultSet rs = pst.executeQuery();
	...
	public int getFavoritesCount(Set<String> complexUserID, String absoluteUserID) {
		...
		try {
			Statement s = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);			
			String sql = " select count(*) from (select docid from maindocs where docid in (select docid from readers_maindocs where username = '" + absoluteUserID + "' and favorites = 1) " +
					" union " +
					" select docid  from tasks where docid in (select docid from readers_tasks where username = '" + absoluteUserID + "' and favorites = 1) " +
					" union " +
					" select docid  from executions where docid in (select docid from readers_executions where username = '" + absoluteUserID + "' and favorites = 1) " +
					" union " +
					" select docid  from projects where docid in (select docid from readers_projects where username = '" + absoluteUserID + "' and favorites = 1)) cnt ";			
			ResultSet rs = s.executeQuery(sql);
			if (rs.next()) {
				count = rs.getInt(1);
			}
